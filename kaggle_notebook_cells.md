...
| `train.py` | Handles configuration, data preparation, two-phase training, checkpointing, evaluation, and batch testing. | **Cells 2, 3, 8, 9, 10, and 11** |

The optional `compare_model/` baselines are not required for the core training loop. You can add new notebook cells after Cell 11 if you wish to port those experiments.
...
def evaluate(model, data_loader, average="macro"):
    model.eval()
    preds = []
    targets = []
    with torch.no_grad():
...
## Cell 10 – Evaluation
Evaluate the trained model on the held-out test set and report accuracy, Cohen’s kappa, precision, recall, and macro F1.
```python
...
test_metrics = evaluate(sst_dpn, test_loader)
print(f"Test Accuracy: {test_metrics['accuracy']:.4f}")
print(f"Test Cohen's Kappa: {test_metrics['kappa']:.4f}")
...
## Cell 11 – Automated BNCI 2b Subject Sweep
Train and evaluate every BNCI 2b subject sequentially. The cell records the train/validation/test sample counts and classification metrics per subject, plus an aggregate row with mean values across the sweep.
```python
import copy
import pandas as pd

def run_subject_experiment(subject_id: int, base_config: dict) -> dict:
    subject_config = copy.deepcopy(base_config)
    subject_config.update({"dataset_id": "2b", "subject_id": subject_id})

    X, y, X_test, y_test = load_bcic(
        subject_config["dataset_id"],
        subject_config["subject_id"],
        subject_config["preprocessing"],
    )

    X_train, X_val, y_train, y_val = train_test_split(
        X,
        y,
        test_size=subject_config["test_size"],
        random_state=subject_config["random_state"],
    )

    num_classes = int(len(np.unique(np.concatenate([y, y_test]))))
    subject_config["num_classes"] = num_classes
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=subject_config["batch_size"],
        shuffle=True,
    )
    val_loader = DataLoader(
        val_dataset,
    ...
    model = SST_DPN(
        chans=X_train_tensor.shape[1],
        samples=X_train_tensor.shape[2],
        num_classes=subject_config["num_classes"],
        pool_kernels=subject_config["mvp_pool_kernels"],
    ).to(device)

    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(
        model.parameters(),
        lr=subject_config["learning_rate"],
        weight_decay=subject_config["weight_decay"],
    )
    optimizer4isp = torch.optim.Adam(
        [{"params": model.isp, "lr": subject_config["lr_isp"]}]
    )
    optimizer4icp = torch.optim.Adam(
        [{"params": model.icp, "lr": subject_config["lr_icp"]}]
    )

    loss_icp = NormIncreaseLoss()
    loss_pl = PrototypeLoss()

    checkpoint_path = WORKING_ROOT / f"best_model_subject_{subject_id}.pth"
...
subjects_to_run = list(range(1, 10))
print("Running BNCI 2b sweep for subjects:", subjects_to_run)

sweep_results = []
for subject in subjects_to_run:
    print(f"\nSubject {subject}...")
    result = run_subject_experiment(subject, CONFIG)
    sweep_results.append(result)

results_df = pd.DataFrame(sweep_results)
average_row = {"subject": "Average"}
for column in [
    "train_count",
    "val_count",
    "test_count",
    ...
    average_row[column] = results_df[column].mean()
...
- Save trained weights to Kaggle output by copying `BEST_MODEL_PATH` (or the per-subject checkpoints) into `/kaggle/working` or `/kaggle/outputs`.
