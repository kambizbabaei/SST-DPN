...
| --- | --- | --- |
| `load_data.py` | Prepares the EEG datasets once they have been downloaded. | **Cell 7 – Data Loading and Dataloaders** |
| `model.py` & `model_utils.py` | Define the SST-DPN architecture and its helper building blocks. | **Cell 5 – Imports and Utility Functions** and **Cell 8 – Model, Losses, and First Phase Training** |
| `dpl_utils.py` | Provides the prototype-learning regularisers used during optimisation. | **Cell 5** and **Cell 6 – Helper Functions** |
| `train.py` | Orchestrates data splitting, two-phase training, checkpointing, and evaluation. | **Cells 3–10**, which follow the same sequence of operations |
...
Download the requested BNCI competition archive directly from the official BCI Competition IV links before relying on MOABB for loading. Enable Kaggle Internet when running this cell.
```python
import shutil
import zipfile
...
if not target_folder.exists() or not any(target_folder.glob("*.gdf")):
    ...
    extracted_dir = MOABB_CACHE / EXTRACTED_FOLDERS[DATASET_ID]
    if extracted_dir.exists() and extracted_dir != target_folder:
        if target_folder.exists():
            shutil.rmtree(target_folder)
        extracted_dir.rename(target_folder)

    print(f"Dataset extracted to {target_folder}")
else:
    print(f"Dataset already available at {target_folder}")
```

---

## Cell 5 – Imports and Utility Functions
```python
import os
import torch
...
## Cell 7 – Data Loading and Dataloaders
```python
# `load_bcic` leverages MOABB/Braindecode and now uses the archives downloaded
# in Cell 4 (mirroring the repository links). The files live under the
# MOABB cache directory configured earlier, so repeated runs reuse the cached
# extraction without triggering new downloads.
...
## Cell 8 – Model, Losses, and First Phase Training
```python
model = SST_DPN(
...
## Cell 9 – Second Phase Training and Evaluation
```python
...
## Cell 10 – (Optional) Persist Metrics
```python
import json
